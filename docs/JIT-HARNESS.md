...
This package expresses **Segment Anything Model** as a typed, task-adaptive agent harness. Its concrete capability remains: Use SAM for zero-shot image segmentation with points, boxes, or masks. The harness contract makes memory, planning, action, capability selection, failure handling, and verification explicit.
...
| Module | Strategy | Responsibility in Segment Anything Model |
| --- | --- | --- |
| Memory | `stage_artifact_log` | Retain source image plus decisions, observations, and verification evidence needed to resume safely. |
| Planning | `linear_gated_plan` | Transform point box or mask prompt into the ordered, bounded stages defined by this skill. |
...
| Action | `brief_generate_render_inspect_export` | Advance the workflow through sam image encoder while preserving stop conditions and user authority. |
